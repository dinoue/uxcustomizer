...
/** Render one security status card. ok===true→good, false→bad, null→unknown. */
$card = static function (array $c) use ($h): string {
    ...
    return '<div class="uxc-card uxc-status-card uxc-' . $state . '">'
        . '<div class="uxc-status-title">' . $h($c['label']) . '</div>'
...
<div class="uxc-ci-detail">
  ...
  <div class="uxc-topbar">
    ...
    <div class="uxc-topbar-badges">
      ...
      <span class="uxc-badge"><?= $h($data['location']) ?></span>
      <span class="uxc-badge"><?= __('Owner', 'uxcustomizer') ?>: <?= $h($data['owner']) ?></span>
      <a class="uxc-btn" href="<?= $h($data['edit_url']) ?>"><?= __('Edit') ?></a>
    </div>
  </div>
  ...
  <div class="uxc-grid uxc-grid-2">
    ...
    <div class="uxc-card">
      <div class="uxc-card-title"><?= __('Software summary', 'uxcustomizer') ?></div>
    ...
    <div class="uxc-card">
      <div class="uxc-card-title"><?= __('Details', 'uxcustomizer') ?></div>
      ...
      <?php else: ?>
        <p class="uxc-muted"><?= __('No custom fields mapped yet.', 'uxcustomizer') ?></p>
      <?php endif; ?>
    ...
    <div class="uxc-card">
      <div class="uxc-card-title"><?= __('Tickets', 'uxcustomizer') ?></div>
      <div class="uxc-metrics">
    ...
    <div class="uxc-card">
      <div class="uxc-card-title"><?= __('Contracts', 'uxcustomizer') ?></div>
      <dl class="uxc-kv">
